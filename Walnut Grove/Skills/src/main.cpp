...
//NEED TO CHANGE
  driveRight(75, 1290);    // Turn right 820 motor degrees
 ...
 if(Controller2.Axis3.value()==0&&!Controller1.ButtonA.pressing()){
  ...
  if(Controller2.ButtonL1.pressing()){
    ...
    if(Controller2.ButtonA.pressing()){
        Lift.setVelocity(100, velocityUnits::pct);
  Lift.rotateFor(-600, rotationUnits::deg, true);
    }   
       if(Controller2.ButtonB.pressing()){
        Lift.setVelocity(100, velocityUnits::pct);
  Lift.rotateFor(-430, rotationUnits::deg, true);
    }   
...
if(count>=150){
  ...
  Controller1.Screen.print("TIMES UP");
  Controller2.Screen.print("TIMES UP");
  Controller1.rumble("...");
   Controller2.rumble("...");
}
if(count>=180){
...
flipOut();
    ...
    // Bring the pusher back when B is pressed
