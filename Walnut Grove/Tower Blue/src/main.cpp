...
void autonomusA() {
  ...
  stopall(10);            // Stop all motors for 10 milliseconds
                          // NEED TO CHANGE
  driveRight(75, 882);   // Turn right 820 motor degrees
...
void usercontrol(void) {
  ...
  while (1) { // While the program is running
    ...
    // If L1 is held down then spin the intake forward
      ...
      IntakeL.spin(directionType::rev, outakeSpeed, vex::velocityUnits::pct);
      IntakeR.spin(directionType::rev, outakeSpeed, vex::velocityUnits::pct);
