...
void usercontrol(void) {
 ...
 thread matchtimer = thread(timer1);
  ...
  //Get all of the controller values so we can calculate the mecanum drive code.
    int Ch1 = Controller1.Axis1.value()/0.7; //1.5
    int Ch3 = Controller1.Axis3.value()/0.7; //2
    int Ch4 = Controller1.Axis4.value()/0.7; //1.5
...
void flipOut(void) { // The function we use to flip out the robot
  IntakeR.spin(vex::directionType::fwd, 127, vex::velocityUnits::pct); // Intake
  IntakeL.spin(vex::directionType::fwd, 127, vex::velocityUnits::pct); // Intake
  Lift.stop(vex::brakeType::hold);
  vex::task::sleep(1000);
  stopall(10); 
...
time = time in milliseconds*/
 ...
 // driveBackward(50,100,300);//drive backwards and hit the wall so that the robot is lined up 
  driveForwardSecOne(65,1160,2000);   //Drive forward and intake the four cubes in front
  driveRight(75, 440, 1200); //Turn so that the intake points to the socring zone BUT is also parallel to it
  strafeLeft(60,500,500);//Hit the wall by strafing  //CALIBRATE POLICE
