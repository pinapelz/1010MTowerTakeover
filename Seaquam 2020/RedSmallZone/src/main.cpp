...
void usercontrol(void) {
 ...
 thread matchtimer = thread(timer1);
  ...
  //Get all of the controller values so we can calculate the mecanum drive code.
    int Ch1 = Controller1.Axis1.value()/0.6; //1.5
    int Ch3 = Controller1.Axis3.value()/0.6; //2
    int Ch4 = Controller1.Axis4.value()/0.6; //1.5
...
flipOut();
 driveBackward(45,50,50);//drive backwards and hit the wall so that the robot is lined up 
  driveForwardSecOne(60,1125,2000);   //Drive forward and intake the 3 cubes in front
  driveRight(60,800,700); //NEEDS TUNING Get in position to back into next set of cubes
  driveBackward(75, 800, 1200); //NEEDS TUNING Drive to the next set of cubes
  driveLeft(75,300,700); //NEEDS TUNING Line up with cubes
  //driveBackward(75,60,50); //Uncomment if we need to line up better (hit the wall)
  driveForwardSecOne(60, 1050, 2000); //Intake Set 2 of cubes
  driveRight(75,900,1000);//Turn to face the scoring zone
  driveForward(50,800,2000);//Drive to zone
  placeCubes(); //Score cubes
